        ...
        http.csrf(AbstractHttpConfigurer::disable).cors(cors -> cors.configurationSource(request -> {
            ...
            return config;
                ...
                .authorizeHttpRequests(auth
                        -> auth.requestMatchers("/api/v1/auth/**", "/swagger-ui/**", "/v3/api-docs/**", "/error",
                                "/favicon.ico", "/.well-known/**", "/api/flashcards/system", "/ws/**", "/ws/info",
                                "/api/v1/payments/vnpay-return", "/api/v1/payments/callback", "/api/v1/payments/notify",
                                "/api/v1/auth/verify-email", "/api/v1/auth/resend-verification", "api/v1/auth/forgot-password")
                        .permitAll().requestMatchers(HttpMethod.GET, "/api/v1/courses/**")
                        .permitAll().requestMatchers(HttpMethod.POST, "/api/v1/payments/create").authenticated()
                        .requestMatchers("/api/v1/admin/**").hasRole("ADMIN")
                        .requestMatchers(HttpMethod.GET, "/api/v1/payments/history")
